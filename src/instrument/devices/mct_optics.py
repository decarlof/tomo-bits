...
class MCTOptics(Device):
    ...
    # Configurable PVs
    lens_select = Cpt(EpicsSignal, "LensSelect")
    camera_select = Cpt(EpicsSignal, "CameraSelect")
    ...
    def __init__(self, *args, macros=None, **kwargs):
        super().__init__(*args, **kwargs)
        self.pv_prefixes = {}
        self.config_pvs = {}
        self.control_pvs = {}
        self.lens_cur = None
        self.camera_cur = None
        self.macros = macros or {}

    def initialize(self, pv_files):
        """Reads PV configuration files and initializes PVs."""
        if not isinstance(pv_files, list):
            pv_files = [pv_files]

        for pv_file in pv_files:
            self.read_pv_file(pv_file)

        self.lens_cur = self.lens_select.get()
        self.camera_cur = self.camera_select.get()

    def read_pv_file(self, pv_file):
        """Reads a file containing a list of EPICS PVs."""
        with open(pv_file, "r") as f:
            lines = f.readlines()

        for line in lines:
            is_config_pv = True
            if "#controlPV" in line:
                line = line.replace("#controlPV", "").strip()
                is_config_pv = False
            ...
            for key, value in self.macros.items():
                pv_name = pv_name.replace(key, value)

            epics_pv = EpicsSignal(pv_name)
            if is_config_pv:
                self.config_pvs[pv_name] = epics_pv
            else:
                self.control_pvs[pv_name] = epics_pv

    def reset_watchdog(self):
        """Sets the watchdog timer to 5 every 3 seconds."""
        while True:
            self.mct_status.put(5)
            time.sleep(3)

    def wait_pv(self, epics_pv, wait_val, timeout=-1):
        """Wait for an EPICS PV to reach a specific value."""
        ...
        while True:
            ...
            if isinstance(pv_val, float):
                if abs(pv_val - wait_val) < EPSILON:
                    return True
            elif pv_val == wait_val:
                return True

            if timeout > -1 and (time.time() - start_time) > timeout:
                raise TimeoutError(
                    f"Timeout waiting for {epics_pv.pvname} to reach {wait_val}"
                )

            time.sleep(0.01)

    def sync_status(self):
        """Synchronizes the device state with the actual motor positions."""
        self.mct_status.put("Sync in progress")

        # Sync logic here (similar to original implementation)
        self.mct_status.put("Done")

    def crop_detector(self):
        """Crop detector sizes."""
        camera_select = str(self.camera_select.get())
        state = self.control_pvs[f"Cam{camera_select}Acquire"].get()
        self.control_pvs[f"Cam{camera_select}Acquire"].put(0, wait=True)

        # Example logic for cropping
        max_size_x = self.control_pvs[f"Cam{camera_select}MaxSizeXRBV"].get()
        self.control_pvs[f"Cam{camera_select}MinX"].put(0, wait=True)

        # Finish cropping logic here

        self.control_pvs[f"Cam{camera_select}Acquire"].put(state)


# Example initialization
# optics = MCTOptics("MY:PV:BASE:", name="optics")
# optics.initialize(["pv_config.txt"])
...
class MCTOptics:
    ...
    def __init__(self, pv_files, macros): #Where these come from: start_mctoptics.py
        ...
        self.control_pvs["Camera0RotationPosition"] = PV(
            camera0_rotation_pv_name + ".VAL"
        )

        camera1_rotation_pv_name = self.control_pvs["Camera1Rotation"].pvname
        self.control_pvs["Camera1RotationPosition"] = PV(
            camera1_rotation_pv_name + ".VAL"
        )
        ...
        self.control_pvs["Cam0ConvertPixelFormat"] = PV(
            camera_prefix + "ConvertPixelFormat"
        )
        ...
        self.control_pvs["Cam1ConvertPixelFormat"] = PV(
            camera_prefix + "ConvertPixelFormat"
        )
    ...
    def sync_binning_select(self, camera_id):
        ...
        if bin_x == 1 or bin_x == 2 or bin_x == 4:
            ...
            log.info(
                "mctOptics: Set camera %s binning to %d",
                camera_id,
                self.epics_pvs["CameraBinning"].get(as_string=True),
            )
    ...
    def sync_bit_select(self, camera_id):
        ...
        # STATE  4:          RGB8        RGB8
        # STATE  5:          RGB16       RGB16
        cam_pixel_format = self.control_pvs[
            "Cam" + str(camera_id) + "PixelFormat"
        ].get()
    ...
    def pv_callback(self, pvname=None, value=None, char_value=None, **kw):
        ...
        log.debug(
            "pv_callback pvName=%s, value=%s, char_value=%s", pvname, value, char_value
        )
        if (pvname.find("LensSelect") != -1) and (
            (value == 0) or (value == 1) or (value == 2)
        ):
    ...
    def take_camera_rotation(self, lens, cam):
        return self.epics_pvs[
            "Camera" + str(cam) + "Lens" + str(lens) + "Rotation"
        ].get()
    ...
    def lens_select(self):
        ...
        self.epics_pvs[
            "Camera" + str(self.camera_cur) + "Lens" + str(self.lens_cur) + "Rotation"
        ].put(rotation_cur)
        ...
        x_select, y_select, z_select = self.take_lens_offsets(
            lens_select, self.camera_cur
        )
        # Take camera id and its rotation for the new lens
        ...
        # Set the correct camera rotation for the lens in use.
        self.epics_pvs["Camera" + str(self.camera_cur) + "RotationPosition"].put(
            camera_rotation
        )  # no wait, assuming the lens movement is the slowest part
        ...
        # Move to the new selected lens
        self.epics_pvs["LensMotor"].put(
            lens_positions[lens_index], wait=True, timeout=120
        )
        ...
        try:
            ...
            log.info(
                "mctOptics: camera %s binning is set to %d",
                str(self.camera_cur),
                pow(2, binning),
            )
            image_pixel_size = (
                float(detector_pixel_size) / float(magnification) * pow(2, binning)
            )
            self.epics_pvs["ImagePixelSize"].put(image_pixel_size)
        except KeyError as e:
            log.error(
                "Lens called %s is not defined. Please add it to the ./data/lens.json file"
                % e
            )
        ...
        try:
            scintillator_type = scintillator_lookup[str(lens_index)][
                "scintillator_type"
            ]
            scintillator_thickness = scintillator_lookup[str(lens_index)][
            ...
            log.error(
                "Scintillator called %s is not defined. Please add it to the ./data/scintillator.json file"
                % e
            )
    ...
    def camera_select(self):
        ...
        lens0_focus_pos = self.epics_pvs[
            "Camera" + str(camera_select) + "Lens0Focus"
        ].get()
        lens1_focus_pos = self.epics_pvs[
            "Camera" + str(camera_select) + "Lens1Focus"
        ].get()
        lens2_focus_pos = self.epics_pvs[
            "Camera" + str(camera_select) + "Lens2Focus"
        ].get()
        ...
        self.control_pvs["Camera" + str(camera_select) + "RotationPosition"].put(
            camera_rotation, wait=True
        )
        ...
        try:
            ...
            magnification = magnification.upper().replace(
                "X", ""
            )  # just in case there was a manual entry ...
            binning = self.epics_pvs["CameraBinning"].get()
            log.info(
                "mctOptics: camera %s binning is set to %d",
                str(camera_select),
            ...
            log.error(
                "Camera called %s is not defined. Please add it to the ./data/camera.json file"
                % e
            )
    ...
    def cross_select(self):
        ...
        if self.epics_pvs["CameraSelect"].get() == 0:
            if self.epics_pvs["CrossSelect"].get() == 0:
                ...
                log.info(
                    "Cross on camera 0 at %d %d is enable" % (sizex // 2, sizey // 2)
                )
            else:
            ...
            if self.epics_pvs["CrossSelect"].get() == 0:
                ...
                log.info(
                    "Cross on camera 1 at %d %d is enable" % (sizex // 2, sizey // 2)
                )
            else:
    ...
    def sync(self):
        ...
        if camera_select != camera_select_sync:
            ...
            log.warning(
                "Sync camera: not required, selector is already in the correct position"
            )
        ...
        if (lens_select != lens_select_sync) and (lens_select_sync != -1):
            ...
            log.warning(
                "Sync lens: not required, selector is already in the correct position"
            )
    ...
    def crop_detector(self):
        ...
        self.epics_pvs["Cam" + camera_select + "SizeX"].put(
            maxsizex - left - right, wait=True
        )
        ...
        self.epics_pvs["Cam" + camera_select + "SizeY"].put(
            maxsizey - top - bottom, wait=True
        )
    ...
    def update_suggested_scan_param(self):
        ...
        if image_size_x > 0:
            ...
            log.error(
                "mctOptics: Suggested scan parameters failed to update: Check camera: %s"
                % str(camera_select)
            )
    ...
    def camera_bit(self, camera_id):
        ...
        log.info(
            "mctOptics: Set camera %s to %s"
            % (
                str(camera_id),
                self.epics_pvs["Camera" + str(camera_id) + "Bit"].get(as_string=True),
            )
        )
        ...
        if bit_selected == 0:
            self.control_pvs["Cam" + str(camera_id) + "GC_AdcBitDepth"].put(
                0, wait=True
            )
            self.control_pvs["Cam" + str(camera_id) + "PixelFormat"].put(0, wait=True)
            self.control_pvs["Cam" + str(camera_id) + "ConvertPixelFormat"].put(
                1, wait=True
            )
        elif bit_selected == 1:
            self.control_pvs["Cam" + str(camera_id) + "GC_AdcBitDepth"].put(
            ...
            if camera_id == 1:
                self.control_pvs["Cam" + str(camera_id) + "PixelFormat"].put(
                    3, wait=True
                )
            else:
                self.control_pvs["Cam" + str(camera_id) + "PixelFormat"].put(
        ...
        if camera_acquire == 1:
            time.sleep(1)
            log.info("restarting the camera")
            self.control_pvs["Cam" + str(camera_id) + "Acquire"].put(
                "Acquire", wait=True
            )
    ...
    def camera_binning(self):
        ...
        binning = self.epics_pvs["CameraBinning"].get()
        log.info(
            "mctOptics: Set camera %s binning to %d", camera_select, pow(2, binning)
        )
