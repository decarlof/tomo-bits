    ...
    def wait_pv(self, epics_pv, wait_val, timeout=-1):
        ...
        while True:
            ...
            if timeout > -1 and (time.time() - start_time) > timeout:
                raise TimeoutError(f"Timeout waiting for {epics_pv.pvname} to reach {wait_val}")
...
class MCTOptics():
    ...
    def __init__(self, pv_files, macros):
        ...
        for epics_pv in ('LensSelect', 'CameraSelect', 'CrossSelect', 'Sync', 'Cut',
                         'Camera0Bit', 'Camera1Bit', 'CameraBinning'):
    ...
    def pv_callback(self, pvname=None, value=None, char_value=None, **kw):
        ...
        elif (pvname.find('Camera0Bit') != -1) and ((value == 0) or (value == 1) or
                                                    (value == 2) or (value == 3)):
            thread = threading.Thread(target=self.camera_bit, args=(0,))
            thread.start()
        elif (pvname.find('Camera1Bit') != -1) and ((value == 0) or (value == 1) or
                                                    (value == 2) or (value == 3)):
            thread = threading.Thread(target=self.camera_bit, args=(1,))
            thread.start()
        elif (pvname.find('CameraBinning') != -1) and ((value == 0) or (value == 1) or
                                                       (value == 2)):
